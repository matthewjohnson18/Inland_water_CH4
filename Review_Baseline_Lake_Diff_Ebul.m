...
load Boreal_Temp_Trop_Baseline_emiss_rates_v04_All_new_data_THETA_TEST.mat;

cd C:\Users\mjohns34\RESEARCH\NASA_Research\Research_Ideas\Methane_Wetland_Lakes_NA\scripts\Future_Lake_emissions\Emission_Rates\;
load Thermokarst_Baseline_emiss_rates_v03_All_new_data_THETA_TEST.mat;
load Glacial_postglacial_Baseline_emiss_rates_v02_THETA_TEST.mat;
load Peat_Pond_Baseline_emiss_rates_v02_THETA_TEST.mat;
...
Thermo_rate = (THERMOKARST_MEAN_DIFFUSIVE_CH4_FLUX_mg_m2_d + THERMOKARST_MEAN_DIFFUSIVE_CH4_FLUX_mg_m2_d)./1000;
Glacial_rate = (GLACIAL_MEAN_DIFFUSIVE_CH4_FLUX_mg_m2_d + GLACIAL_MEAN_DIFFUSIVE_CH4_FLUX_mg_m2_d)./1000;
Peat_rate = (PEAT_POND_MEAN_DIFFUSIVE_CH4_FLUX_mg_m2_d + PEAT_POND_MEAN_EBULLITIVE_CH4_FLUX_mg_m2_d)./1000;
...
clear Thermo_rate_interp Glacial_rate_interp Peat_rate_interp Organic_rate_north_interp Organic_rate_south_interp Boreal_rate_interp Temperate_north_rate_interp Temperate_south_rate_interp Tropical_north_rate_interp Tropical_south_rate_interp;

type1_daily = single(CH4_emiss_Lake_Type1_g_m2_day);
type2_daily = single(CH4_emiss_Lake_Type2_g_m2_day);
type3_daily = single(CH4_emiss_Lake_Type3_g_m2_day);
type4_daily = single(CH4_emiss_Lake_Type4_g_m2_day);
type5_daily = single(CH4_emiss_Lake_Type5_g_m2_day);
type6_daily = single(CH4_emiss_Lake_Type6_g_m2_day);
type7_daily = single(CH4_emiss_Lake_Type7_g_m2_day);
...
for j = 1:J_GLOB;
    for k = 1:K_GLOB;
        ...
        Accum_days(j,k) = Freeze_length(j,k)-75;
        if(Accum_days(j,k)<0)
            Accum_days(j,k)=0.0;
        end;
...
clear Season_length_test Accum_days_type1 Accum_days_type2 Accum_days_type3 Accum_days_type4 Accum_days_type5 Accum_days_type6 Accum_days_type7 Accum_days Freeze_length;
...
Burst_length = 14;
Oxidation_fraction = 0.40; % 40% of accumulated methane is oxidized.
...
for j = 1:J_GLOB;
    for k = 1:K_GLOB;
        if( h>=Thaw_Date_check(j,k) && h<=Freeze_Date_check(j,k) )
            CH4_emiss_Lake_Type1_g_m2_day(h,j,k) = CH4_emiss_Lake_Type1_g_m2_day(h,j,k)+type1_daily(h,j,k);
            CH4_emiss_Lake_Type2_g_m2_day(h,j,k) = CH4_emiss_Lake_Type2_g_m2_day(h,j,k)+type2_daily(h,j,k);
            CH4_emiss_Lake_Type3_g_m2_day(h,j,k) = CH4_emiss_Lake_Type3_g_m2_day(h,j,k)+type3_daily(h,j,k);
            CH4_emiss_Lake_Type4_g_m2_day(h,j,k) = CH4_emiss_Lake_Type4_g_m2_day(h,j,k)+type4_daily(h,j,k);
            CH4_emiss_Lake_Type5_g_m2_day(h,j,k) = CH4_emiss_Lake_Type5_g_m2_day(h,j,k)+type5_daily(h,j,k);
            CH4_emiss_Lake_Type6_g_m2_day(h,j,k) = CH4_emiss_Lake_Type6_g_m2_day(h,j,k)+type6_daily(h,j,k);
            CH4_emiss_Lake_Type7_g_m2_day(h,j,k) = CH4_emiss_Lake_Type7_g_m2_day(h,j,k)+type7_daily(h,j,k);
        end;
    end;
end;
end;
