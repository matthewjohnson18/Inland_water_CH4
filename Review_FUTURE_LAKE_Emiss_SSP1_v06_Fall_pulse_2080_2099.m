...
%%%%%%%%% Original Emission Rates (Johnson et al 2022) %%%%%%%%%%%%%%%%%%
...
clear Large_lake_mask Lake_Area_m2_01_1 Lake_Area_m2_1_10 Lake_Fraction_01_1k Lake_Fraction_1_10k Lake_Area_m2_5000k Lake_Fraction_5000k Lake_Area_m2_5_10k Lake_Fraction_5_10k Lake_Area_m2_10_100k Lake_Fraction_10_100k Lake_Area_m2_100k_greater Lake_Fraction_100k_greater;
...
for j = 1:J_GLOB;
    for k = 1:K_GLOB;
        if( (CH4_EM_Lake_Type(j,k) == 1) )
            CH4_EM_Lake_Season_GLOB_TYPE1(j,k) = Lake_Season_SSP1(j,k);
        end;
        if( (CH4_EM_Lake_Type(j,k) == 2) )
            CH4_EM_Lake_Season_GLOB_TYPE2(j,k) = Lake_Season_SSP1(j,k);
        end;
        if( (CH4_EM_Lake_Type(j,k) == 3) )
            CH4_EM_Lake_Season_GLOB_TYPE3(j,k) = Lake_Season_SSP1(j,k);
        end;
        if( (Lake_Type_TYPE4_Area(j,k) > 0) )
            CH4_EM_Lake_Season_GLOB_TYPE4(j,k) = Lake_Season_SSP1(j,k);
        end;
        if( (Lake_Type_TYPE5_Area(j,k) > 0) )
            CH4_EM_Lake_Season_GLOB_TYPE5(j,k) = Lake_Season_SSP1(j,k);
        end;
        if( (Lake_Type_TYPE6_Area(j,k) > 0) )
            CH4_EM_Lake_Season_GLOB_TYPE6(j,k) = Lake_Season_SSP1(j,k);
        end;
        if( (Lake_Type_TYPE7_Area(j,k) > 0) )
            CH4_EM_Lake_Season_GLOB_TYPE7(j,k) = Lake_Season_SSP1(j,k);
        end;
    end;
end;
...
Thaw_Date_check = Thaw_Date_SSP1;
Thaw_Date_check(Lake_Season_SSP1==365)=0.0;
Freeze_Date_check = Freeze_Date_SSP1;
Freeze_Date_check(Lake_Season_SSP1==365)=365;
...
FallTurn_flux_Type1_g_m2 = zeros(720,1440);
FallTurn_flux_Type2_g_m2 = zeros(720,1440);
FallTurn_flux_Type3_g_m2 = zeros(720,1440);
FallTurn_flux_Type4_g_m2 = zeros(720,1440);
FallTurn_flux_Type5_g_m2 = zeros(720,1440);
FallTurn_flux_Type6_g_m2 = zeros(720,1440);
FallTurn_flux_Type7_g_m2 = zeros(720,1440);
Season_length_test = Lake_Season_SSP1;
Season_length_test(isnan(Season_length_test))=0.0;

Lag_time = 60; % lag time for accumulation to start;


for j = 1:J_GLOB;
    for k = 1:K_GLOB;
        Freeze_length(j,k) = 365-Season_length_test(j,k);
        Accum_days(j,k) = Season_length_test(j,k)-Lag_time;
        if(Accum_days(j,k)<0)
            Accum_days(j,k)=0.0;
        end;
        if(Accum_days(j,k)>364)
            Accum_days(j,k)=0.0;
        end;
        ...
        FallTurn_flux_Type1_g_m2(j,k) = Accum_days_type1(j,k).*nanmean(Thermo_rate);
        FallTurn_flux_Type2_g_m2(j,k) = Accum_days_type2(j,k).*nanmean(Glacial_rate);
        FallTurn_flux_Type3_g_m2(j,k) = Accum_days_type3(j,k).*nanmean(Peat_rate);
        FallTurn_flux_Type4_g_m2(j,k) = Accum_days_type4(j,k).*nanmean(Organic_rate_north);
        FallTurn_flux_Type5_g_m2(j,k) = Accum_days_type5(j,k).*nanmean(Boreal_rate);
        FallTurn_flux_Type6_g_m2(j,k) = Accum_days_type6(j,k).*nanmean(Temperate_north_rate+Temperate_south_rate);
        FallTurn_flux_Type7_g_m2(j,k) = Accum_days_type7(j,k).*nanmean(Tropical_north_rate+Tropical_south_rate);           
	end;
end;
...
CH4_emiss_Lake_Type6_g_m2_day = zeros(365,J_GLOB,K_GLOB);
CH4_emiss_Lake_Type7_g_m2_day = zeros(365,J_GLOB,K_GLOB);
Turnover_Pulse_g_m2_day = zeros(365,J_GLOB,K_GLOB);

Burst_length = 7;
Oxidation_fraction = 0.89; % 89% of accumulated methane is oxidized.

for h = 1:(365-8);
    display(h)
for j = 1:J_GLOB;
    for k = 1:K_GLOB;
        if( (Accum_days(j,k)>0) && (h >= (Freeze_Date_check(j,k)-(Burst_length))) && (h <= (Freeze_Date_check(j,k))) )
            CH4_emiss_Lake_Type1_g_m2_day(h,j,k) = (FallTurn_flux_Type1_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction);
            CH4_emiss_Lake_Type2_g_m2_day(h,j,k) = (FallTurn_flux_Type2_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction);
            CH4_emiss_Lake_Type3_g_m2_day(h,j,k) = (FallTurn_flux_Type3_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction);
            CH4_emiss_Lake_Type4_g_m2_day(h,j,k) = (FallTurn_flux_Type4_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction);
            CH4_emiss_Lake_Type5_g_m2_day(h,j,k) = (FallTurn_flux_Type5_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction);
            CH4_emiss_Lake_Type6_g_m2_day(h,j,k) = (FallTurn_flux_Type6_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction);
            CH4_emiss_Lake_Type7_g_m2_day(h,j,k) = (FallTurn_flux_Type7_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction);
            Turnover_Pulse_g_m2_day(h,j,k) = ((FallTurn_flux_Type1_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction))+((FallTurn_flux_Type2_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction)) ...
                                         +((FallTurn_flux_Type3_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction))+((FallTurn_flux_Type4_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction)) ...
                                         +((FallTurn_flux_Type5_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction))+((FallTurn_flux_Type6_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction))+((FallTurn_flux_Type7_g_m2(j,k)./Burst_length).*(1-Oxidation_fraction));
        end;
    end;
end;
end;
...
fall_pulse(:,:)=nansum(Turnover_Pulse_g_m2_day);
fall_pulse(isnan(fall_pulse))=0.0;
for j = 1:J_GLOB;
    for k = 1:K_GLOB;
...
DAILY_FALL_Pulse_g_m2_per_cell = fall_pulse.*Lake_Fraction;
DAILY_FALL_Pulse_g_m2_per_cell(DAILY_FALL_Pulse_g_m2_per_cell==0.0)=NaN;
...
DAILY_FALL_Pulse_g_m2(:,:) = nansum(Turnover_Pulse_g_m2_day);
...
DAILY_LAKE_EMISSION_CH4_Tg_yr_FALL_PULSE = nansum(nansum(DAILY_FALL_Pulse_g_m2.*area_m2.*Lake_Fraction)).*10^-12;
